...
class customInformationCard: NSObject, UIScrollViewDelegate, UITextFieldDelegate {
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        if validateText(str: newNick ?? "") {
            var nickArray = data.object(forKey: "Aliases") as? [String:String] ?? [:]
            nickArray[PermaCode!] = newNick
            data.setValue(nickArray, forKey: "Aliases")
    ...
    func showAlert(userCode: String, viewController: UIViewController) {
        PermaCode = userCode
